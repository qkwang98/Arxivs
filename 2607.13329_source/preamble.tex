...
\hypersetup{
	colorlinks=true, % 使用颜色而不是方框来标记链接
	linkcolor=purple,  % 内部链接的颜色
	citecolor=lightgreen, % 引用的颜色
	urlcolor=cyan,   % URL 的颜色
}
\usepackage{pdfpages}
...
\usepackage[backend=biber,style=alphabetic,maxnames=99,giveninits=true]{biblatex}  % 使用 Biber
\DeclareFieldFormat[article,unpublished,misc]{title}{#1} % remove quotes
\addbibresource{doc.bib}  % 指定你的 .bib 文件（确保与实际文件名匹配）
...
\renewcommand{\theThmAlpha}{\Alph{ThmAlpha}} % "letter-numbered" theorems
...
\newcommand{\theconvention}{}  % to make the notation environment unnumbered
...
% 定义矩阵内容宏
\newcommand{\matone}{\ensuremath{\bigl(\begin{smallmatrix}1 & * \\ 0 & 1\end{smallmatrix}}\bigr)}
\newcommand{\matoneF}{\ensuremath{\bigl(\begin{smallmatrix}1 & * \\ 0 & 1\end{smallmatrix}}\bigr)}
