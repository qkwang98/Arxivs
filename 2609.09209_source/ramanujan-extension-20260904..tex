...
\vskip 1cm{\large\bf  
Generalizations of two Ramanujan series for $1/\pi$}
\vskip 1cm
\large
Kunle Adegoke \\ 
...
\begin{abstract}
By utilizing two known hypergeometric summation identities we extend two \mbox{well-known} Ramanujan series for $1/\pi$ by making each of them a member of an infinite family of series. We also find the corresponding families involving harmonic numbers and odd harmonic numbers. 
\end{abstract}
...
\sum_{k = 0}^\infty  {( - 1)^k \frac{{\left( {\frac{1}{2}} \right)_k^3 }}{{k!^3 }}\frac{{\left( {4k + 1} \right)}}{{\prod_{j = 1}^n {\left( {k + j} \right)\left( {2k - 2j + 1} \right)} }}}  = \frac{{( - 1)^n }}{{\left( {\frac{1}{2}} \right)_n^2 2^{n - 1} }}\,\frac{1}{\pi }
\end{equation}
and
...
\end{equation}
valid for $n$ a non-negative integer.
...
Let $H_j$ be the $j^{\rm th}$ harmonic number and let $O_j$ be the $j^{\rm th}$ odd harmonic number. We will also derive the corresponding series involving harmonic and odd harmonic numbers, that is,
...
The identity~\eqref{gen1} was first derived by Levrie~\cite[Theorem 7]{levrie10} using a technique that is completely different from our approach. In fact we will see that identities~\eqref{gen1} and~\eqref{gen2} are particular cases of more general results.
...
Lemmata~\ref{edacukd} and~\ref{nnoqyfq} were derived by Hou et al.~\cite{hou24} by making appropriate substitutions in Dougall's well-poised hypergeometric series. They facilitate the derivation of~\eqref{gen1} and~\eqref{gen2}.
...
\end{lemma}
\begin{remark}
The special case of~\eqref{o68u3j7} with $n$ a non-negative integer was proved by Ekhad and Zeilberger~\cite{ekhad94} using the $WZ$ technique.
\end{remark}
\begin{theorem}
...
\end{theorem}
\begin{proof}
Set $c=1/2-r$ in~\eqref{o68u3j7} to obtain
\begin{equation}\label{h7933kq}
\sum_{k = 0}^\infty  {( - 1)^k \frac{{\left( {\frac{1}{2}} \right)_k^2 \left( {\frac{1}{2} - r} \right)_k }}{{k!^2 \left( {r + 1} \right)_k }}\left( {4k + 1} \right)}  = \frac{2}{{\sqrt \pi  }}\left( {r + \frac{1}{2}} \right)_{1/2} .
\end{equation}
Use~\eqref{l5fmdri} and note from~\eqref{ex_pochhammer} that
\begin{equation}\label{nly5agg}
\left( {r + 1} \right)_k  = k!\binom{{r + k}}{k}
\end{equation}
and
...
\end{corollary}
\begin{proof}
Set $r=n$ in~\eqref{h7933kq} and use~\eqref{mq7ny8m},~\eqref{nly5agg} and~\eqref{g20r5hg}. Note that
\begin{equation}
n!\binom{{k + n}}{n} = \prod_{j = 1}^n {\left( {k + j} \right)} .
\end{equation}
\end{proof}
\begin{theorem}
If $r$ is a complex number that is not a half-integer, then
...
\end{equation}
from which~\eqref{r1kycoj} follows after using`\eqref{nerd5x1},~\eqref{l5fmdri},~\eqref{nly5agg} and~\eqref{g20r5hg}.
\end{proof}
\begin{corollary}
If $n$ is a non-negative integer, then
\begin{equation*}
