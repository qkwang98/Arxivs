...
def main():
    ...
    parser.add_argument('--reuse-rk-trials',action='store_true')
    ...
    trials = {}
    if args.reuse_rk_trials:
        if reused is None or reused['digits'] != 70 or reused['points'] != POINTS:
            raise ValueError('RK reuse requires a matching contour/background source')
        for t in POINTS:
            if relative(decode(reused['reference_values'][str(t)]),reference.matrix(t)) > 1e-30:
                raise ValueError('Reused RK background/reference does not match')
        for key,record in reused['rk_trials'].items():
            data['rk_trials'][key] = record
            trials[key] = {t:decode(record['F'][str(t)]) for t in POINTS}
        data['reused_rk_tolerances'] = list(trials)
    def trial(tol):
    ...
    for radius in args.radii:
        ...
        for _ in range(12):
            ...
            if 1/args.match_factor <= achieved/target <= args.match_factor:
                break
            below = [key for key,row in data['rk_trials'].items() if row['max_solution_error'] < target]
            above = [key for key,row in data['rk_trials'].items() if row['max_solution_error'] > target]
            if below and above:
                left,right = min(((a,b) for a in below for b in above),
                                 key=lambda pair:abs(math.log(float(pair[0])/float(pair[1]))))
                el = data['rk_trials'][left]['max_solution_error']
                er = data['rk_trials'][right]['max_solution_error']
                fraction = min(.98,max(.02,math.log(target/el)/math.log(er/el)))
                candidate = mp.exp((1-fraction)*mp.log(mp.mpf(left))+fraction*mp.log(mp.mpf(right)))
                if mp.nstr(candidate,14) in trials:
                    candidate = mp.sqrt(mp.mpf(left)*mp.mpf(right))
            else:
                correction = min(1000,max(.001,target/achieved))
                candidate = mp.mpf(selected)*mp.mpf(str(correction))
            trial(candidate)
        selected = min(data['rk_trials'],key=lambda key:abs(math.log(data['rk_trials'][key]['max_solution_error']/target)))
