...
def main():
    ...
    for radius in (13.5,14.):
        name = f'R{radius}_N40_tol1e-26'
        print('Contour',name,flush=True)
        start = perf_counter()
        method = anchored.AnchoredContours(reference,radius,40,'1e-26',anchors)
        values,diagnostics = {},{}
        for t in POINTS:
        ...
        record = {**metrics(values,reference,jac),'radius':radius,'formal_order':40,
                  'spectral_tolerance':'1e-26','digits':mp.dps,
        ...
        data['pairs'][name] = {'rk_tolerance':selected,'matched':2/3 <= ratio <= 1.5,
