...
def main():
    ...
    parser.add_argument('--spectral-tolerances', nargs='+', default=['1e-6','1e-9','1e-12'])
    ...
    data = {'equation':'u_xx=2*u^3+x*u; alpha=0', 'initial':{'x0':'-1','u0':'.2','w0':'.1'},
            ...
            'match_factor':args.match_factor,'contours':{},'rk_trials':{},'pairs':{},'status':'running'}
    ...
    if args.reuse_monitor:
        ...
        if old['points'] != POINTS or old['initial'] != data['initial']:
            raise ValueError('Monitor background/points mismatch')
        for t in POINTS:
            if relative(decode(old['reference']['F'][str(t)]),ref.matrix(t)) > 1e-32:
                raise ValueError('Monitor reference does not match')
            record = old['monitor'][str(t)]
            jac[t],stokes[t] = decode(record['J']),decode(record['s'])
        data['monitor'] = old['monitor']
        data['monitor_refinement_at_minus13'] = old['monitor_refinement_at_minus13']
    ...
    trials = {}
    reused = json.loads(args.reuse_results.read_text()) if args.reuse_results else None
    if reused:
        if reused['initial'] != data['initial'] or reused['points'] != POINTS or reused['digits'] != 70:
            raise ValueError('Reused comparison settings mismatch')
        if (args.reuse_results.parent/'pii_core.py').read_bytes() != (ROOT/'pii_core.py').read_bytes():
            raise ValueError('Reused contour implementation differs')
        for t in POINTS:
            if relative(decode(reused['reference']['F'][str(t)]),ref.matrix(t)) > 1e-32:
                raise ValueError('Reused comparison reference mismatch')
        data['rk_trials'] = reused['rk_trials']
        trials = {key:{float(t):decode(f) for t,f in record['F'].items()}
                  for key,record in reused['rk_trials'].items()}
        data['reused_results_sha256'] = hashlib.sha256(args.reuse_results.read_bytes()).hexdigest()
    def trial(tol):
    ...
    for tolerance in args.spectral_tolerances:
        ...
        if reused and tolerance in reused['contours']:
            record = dict(reused['contours'][tolerance], timing_reused=True)
        ...
        tol = mp.mpf(min(data['rk_trials'],key=lambda k: abs(mp.log(mp.mpf(data['rk_trials'][k]['max_solution_error'])/target)))) if data['rk_trials'] else target/10
