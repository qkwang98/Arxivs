...
def background(span, tol=3e-14):
    def rhs(t, state):
        q, h, logd = state
        return np.r_[core.qh_rhs(core.X0+t, [q, h]), core.X0+t+q/2]
    initial = [core.Q0, core.H0, 0j]
...
def spectral_data(x, q, h, radius, tol):
...
def contour_matrix(grid, bg, radius, tol):
    start = perf_counter()
    indices = tuple(range(-1, 5))
    outer = {j: radius*np.exp(.5j*np.pi*j) for j in indices}
    initial = np.concatenate([core.subdominant_initial(radius, .5*np.pi*j)
                              for j in indices])
    def rhs(t, state):
        q, h = bg(t)[:2]
    ...
    return result, {"seconds": perf_counter()-start,
                    ...
                    "max_quad_error_estimate": max(quadrature_errors)}
...
def spectral_profile(grid, bg, radius, tol):
    start = perf_counter()
    stokes, invariants, jacobians = [], [], []
    for t in grid:
        q, h, logd = bg(t)
        s, m, jac = spectral_data(core.X0+t, q, h, radius, tol)
...
def main():
    ...
    start = perf_counter()
    bg = background(args.span)
    background_seconds = perf_counter()-start
    reference, ref_cost = rk_matrix(grid, bg, 3e-14, "DOP853")
    profiles, profile_info = {}, {}
    for radius in (4.8, 6., 7.2):
        print(f"Spectral profile R={radius}", flush=True)
        stokes, m, jac, seconds = spectral_profile(grid, bg, radius, 2e-12)
        profiles[radius] = jac
        mid = len(grid)//2
        profile_info[str(radius)] = {"seconds": seconds,
    ...
    for method, tol in (("RK45", 1e-3), ("RK45", 1e-6), ("RK45", 1e-9),
                         ("contour", 1e-8), ("contour", 1e-10), ("contour", 3e-13)):
        label = f"{method}_{tol:g}"
        print(label, flush=True)
        if method == "contour":
            matrix, cost = contour_matrix(grid, bg, 4.8, tol)
        else:
            matrix, cost = rk_matrix(grid, bg, tol, method)
        matrices[label] = matrix
        row = {"method": method, "tolerance": tol, **cost,
    ...
    report = {"status": "pilot; no certified error bounds or speed claims",
    ...
    np.savez(args.output/"arrays.npz", grid=grid, jacobian=jac,
             **{f"J_R{r}": j for r, j in profiles.items()}, **matrices)
    with (args.output/"comparison.csv").open("w", newline="") as f:
        fields = ["method", "tolerance", "seconds", "solution_error_vs_dop853",
