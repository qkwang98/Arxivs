...
def canonical(x, u, w, index, radius=4, formal_order=40, tolerance='1e-26',
    ...
    while abs(z) > mp.mpf('1e-60'):
        ...
        kernel = kernel_coefficients(z, cf, u) if quadrature else None
        controlled = cf
        if kernel is not None:
            # Products have faster phase variation than individual columns.
            # Include both accumulated integrals in Taylor step selection.
            controlled = cf+[[integrals[k]]+[v/(n+1) for n,v in enumerate(seq)]
                             for k,seq in enumerate(kernel)]
        size = min(step_radius(controlled, mp.mpf(tolerance), '.4'), abs(z))
        step = -z/abs(z)*size
        if quadrature:
            for k, seq in enumerate(kernel):
