...
def contour_matrix(grid, bg, radius, tol, balanced=False):
    ...
    initial_columns = [core.subdominant_initial(radius, .5*np.pi*j) for j in indices]
    if balanced:
...
def main():
    ...
    for radius, tol in ((4.8, 2e-12), (6., 2e-12), (7.2, 2e-12), (7.2, 2e-13)):
        profile_name = f"R{radius:g}_tol{tol:g}"
        print(f"Spectral profile {profile_name}", flush=True)
        stokes, m, jac, seconds = spectral_profile(grid, bg, radius, tol)
    ...
    np.savez(args.output/"arrays.npz", grid=grid, jacobian=jac,
             **{f"J_{name}": j for name, j in profiles.items()}, **matrices)
