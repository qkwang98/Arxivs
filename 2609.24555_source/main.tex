...
\usepackage{capt-of}
\input{appendix_style}
\newcommand{\todo}[1]{\textcolor{red}{[#1]}}
\input{tables/result_numbers.tex}
\input{tables/tool_numbers.tex}
...
To assess current model performance on these construction problems, we evaluate eight models in twelve tool-free configurations: GPT-6 Astra, GPT-5.6 Luna, Claude Fable 5.1, DeepSeek V4.1 Flash, Qwen3.8-27B and Qwen3.5 at 4B, 9B and 27B. To examine the effect of tool access, we additionally evaluate Astra high and Luna high with code and web access on the 69 instances.
...
\caption{Results on 69 instances. The final two rows use code and web access (Section~\ref{sec:protocol}). Overall score is uncapped. Brackets show $95\%$ confidence intervals. Validity uses all 69 responses; gap closed uses \GapN{} instances with proven bounds (Section~\ref{sec:headroom}).}
\label{tab:headline}
\end{table}
...
On the \PanelOneN{} instances with published frontiers, tool-assisted Astra and Luna match \ToolPublishedMatched{} and \LunaToolPublishedMatched{} references, respectively, without surpassing any. Reaching published constructions is thus already observed across much of this group, while improving them remains a further challenge. On the \PanelTwoN{} instances chosen outside published construction tables, Astra and Luna improve on the construction baselines for \ToolConstructionAbove{} and \LunaToolConstructionAbove{} instances (Appendix~\ref{app:tools}). These improvements already demonstrate scoring beyond $1$; future improvements over published frontiers would be measured in the same way.



\subsection{Quality as problem size increases}
\label{sec:ladder}
With tools at high effort, mean gap closed remains small: $\ToolGapClosedPct\%$ for GPT-6 Astra and $\LunaToolGapClosedPct\%$ for GPT-5.6 Luna across the \GapN{} instances with proven bounds. Thus, exceeding some references still leaves most of the measured logarithmic gap to those bounds unclosed. These results place current models near the reference end of this scale, with mathematical bounds providing longer-term targets.
